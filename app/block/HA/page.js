...
export default function HARoomMaintenance() {
  const [loginOpen, setLoginOpen] = useState(false);
  const [authTab, setAuthTab] = useState("login");
  const [complaintOpen, setComplaintOpen] = useState(false);
  const [rooms, setRooms] = useState(() => generateRooms("HA"));
  ...
  const [signupMsgType, setSignupMsgType] = useState("");
  const [forgotEmail, setForgotEmail] = useState("");
  const [forgotMsg, setForgotMsg] = useState("");
  const [forgotMsgType, setForgotMsgType] = useState("");
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-between">
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            ...
            {authTab === "signup" && (
              <form onSubmit={handleSignup} className="px-6 py-4">
                ...
                {signupMsg && <div className={`mb-2 text-center ${signupMsgType === 'error' ? 'text-red-600' : 'text-green-600'}`}>{signupMsg}</div>}
            ...
            {authTab === "forgot" && (
              <form onSubmit={handleForgot} className="px-6 py-4">
                ...
                {forgotMsg && <div className={`mb-2 text-center ${forgotMsgType === 'error' ? 'text-red-600' : 'text-green-600'}`}>{forgotMsg}</div>}
      ...
      <main className="relative flex-1">
        ...
        <div className="max-w-7xl mx-auto relative z-10 px-4 py-10">
          <h1 className="text-3xl font-bold text-gray-900 mb-8">HA Room Maintenance</h1>
